      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-8 ml-20 flex-1">
          <Link
            ...
            to={getAppUrl({type: 'docs', lang, DEFAULT_LANGUAGE, LATEST_VERSION})}>
            {t(l => l.header.docs)}
          </Link>
          <Link
            className="hover:text-primary font-semibold"
            to={getAppUrl({type: 'blog', lang, DEFAULT_LANGUAGE, LATEST_VERSION})}>
            {t(l => l.header.blog)}
          </Link>
...
export function AppLayout({linksTree, children}: AppLayoutProps) {
  return (
