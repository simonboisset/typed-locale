...
export function SearchBar({items, trigger}: {items: SearchItem[]; trigger: React.ReactNode}) {
        ...
        <div className="flex flex-col">
          ...
          {results.length > 0 ? (
            <ScrollArea className="h-[calc(50vh-100px)]">
              <div className="flex flex-col">
                {results.map(item => (
                  <SearchItem key={item.item.slug} result={item} onSelect={() => onSelect(item)} />
                ))}
              </div>
            </ScrollArea>
          ) : (
...
const SearchItem = ({result, onSelect}: {result: FuseResult<SearchItem>; onSelect: () => void}) => {
  return (
    <button onClick={onSelect} className="w-full flex flex-col gap-2 items-start hover:bg-accent p-2 transition-colors">
      <span className="font-bold">{result.item.title}</span>
      <HighlightedText text={result.item.content} matches={result.matches?.filter(m => m.key === 'content') ?? []} />
    </button>
  );
};

const getFirstHighlightedText = (text: string, matches: FuseResultMatch[] | undefined) => {
  const distance = 100;
  if (!matches)
    return {
      before: '',
...
const HighlightedText = ({text, matches}: {text: string; matches: FuseResultMatch[]}) => {
  const {before, highlight, after, isStart, isEnd} = getFirstHighlightedText(text, matches);
  return (
    <span className="text-muted-foreground text-start text-sm">
      {isStart && '...'}
      {markdownToPlainText(before)}
      <mark className="bg-primary text-card px-1 py-0.5 rounded-sm font-bold">{highlight}</mark>
