...
const variants = cva(
  'flex w-full bg-transparent transition-colors file:border-0 file:bg-transparent file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50',
  {
    variants: {
      variant: {
        default: 'border border-input shadow-sm focus-visible:ring-1 focus-visible:ring-ring rounded-md',
...
export type InputProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size'> & VariantProps<typeof variants>;

const Input = React.forwardRef<HTMLInputElement, InputProps>(({className, type, variant, size, ...props}, ref) => {
  return <input type={type} className={cn(variants({variant, size}), className)} ref={ref} {...props} />;
});
