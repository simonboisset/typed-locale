...
export const getBlogPosts = (lang: Language): LinkTree[] => {
  return blog[lang]
    .map(post => ({
      href: getAppUrl({DEFAULT_LANGUAGE, lang, LATEST_VERSION, type: 'blog', slug: post.slug}),
...
export const requireBlogPost = (params: Params) => {
  ...
  const redirectUrl = getAppUrl({type: 'blog', lang, slug, DEFAULT_LANGUAGE, LATEST_VERSION});
  ...
  if (!posts) {
    throw redirect(getAppUrl({type: 'blog', lang: validLanguage, LATEST_VERSION, DEFAULT_LANGUAGE}));
  }
