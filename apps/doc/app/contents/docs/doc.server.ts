...
export const getDocs = ({version, language}: {version: string; language: string}): Record<string, string> | null => {
  const versionKey = `v${version}`.replaceAll('.', '_') as keyof typeof doc;
  const docWithVersion = doc[versionKey] as Record<Language, Record<string, string>> | undefined;
  if (!docWithVersion) {
    return null;
  }
  const docWithLanguage = docWithVersion[language as Language] as Record<string, string> | undefined;
  if (!docWithLanguage) {
    return null;
  }
...
export const getDocSummaries = ({
  ...
  lang: Language;
  version: Version;
}): {flat: FlatSummary[]; tree: LinkTree[]; lang: Language; version: Version} => {
  ...
  tree.push(...buildTree({lang, version, items: flat, parent: '', used: []}).tree);
...
const buildTree = ({
  ...
  for (const item of items) {
    if (item.number.startsWith(parent) && item.number !== parent && !allreadyUsed.includes(item.number)) {
      const {tree: children, added} = buildTree({lang, version, items, parent: item.number, used: allreadyUsed});
      const child: LinkTree = {
        href: getAppUrl({type: 'docs', lang, version, slug: item.slug, DEFAULT_LANGUAGE, LATEST_VERSION}),
        title: item.title,
        children,
        slug: item.slug,
      };
...
export const requireDoc = (params: Params) => {
  ...
  const redirectUrl = getAppUrl({type: 'docs', lang, version, slug, DEFAULT_LANGUAGE, LATEST_VERSION});
  const {success: successLang, data: validLanguage} = languageSchema.safeParse(lang || DEFAULT_LANGUAGE);
  const {success: successVersion, data: validVersion} = versionSchema.safeParse(version || LATEST_VERSION);
  const mustRedirect = lang === DEFAULT_LANGUAGE || !successLang || !successVersion || version === LATEST_VERSION;
  ...
  if (!docs) {
    throw redirect(
      getAppUrl({type: 'docs', lang: validLanguage, version: validVersion, LATEST_VERSION, DEFAULT_LANGUAGE}),
    );
  }
