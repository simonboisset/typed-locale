...
export const loader = async ({params}: LoaderFunctionArgs) => {
  ...
  if (!summary) {
    throw redirect(getAppUrl({type: 'docs', lang, version, DEFAULT_LANGUAGE, LATEST_VERSION}));
  }

  const doc = docs[summary.number];
  const nextArticle = summaries.flat[summaries.flat.findIndex(doc => doc.number === summary.number) + 1];
  const previousArticle = summaries.flat[summaries.flat.findIndex(doc => doc.number === summary.number) - 1];
