...
test('Should translate correctly translator with plural', () => {
  ...
  expect(translateEn((l) => l.helloNameYouHaveMessages({ count: 0, name: 'Jo' }))).toBe(
    'Hello, Jo. You have no messages',
  );
  expect(translateEn((l) => l.helloNameYouHaveMessages({ count: 1, name: 'Jo' }))).toBe(
    'Hello, Jo. You have 1 message',
  );
  expect(translateEn((l) => l.helloNameYouHaveMessages({ count: 2, name: 'Jo' }))).toBe(
